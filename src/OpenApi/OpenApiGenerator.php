...
class OpenApiGenerator
{
    ...
    public function generate(?string $version = null): array
    {
        ...
        $this->initializeSpec($version);
        $this->processRoutes($version);
    ...
    {
        $versioningConfig = config('api-response.openapi.versioning', []);

        if (! ($versioningConfig['enabled'] ?? false)) {
            return [];
        }

        if ($versioningConfig['auto_detect'] ?? true) {
            return $this->detectVersionsFromRoutes();
        }

        return $this->getConfiguredVersions();
    }

    /**
     * Auto-detect versions from route prefixes.
     */
    protected function detectVersionsFromRoutes(): array
    {
        ...
        foreach ($routes as $route) {
            ...
            // Match version pattern like api/v1, api/v2, etc.
            if (preg_match('/^' . preg_quote($prefix, '/') . '\/v(\d+)/', $uri, $matches)) {
                $version = 'v' . $matches[1];
                if (! isset($versions[$version])) {
                    $versions[$version] = [
                        'name' => $version,
                        'title' => 'API ' . strtoupper($version),
                        'description' => 'API version ' . $matches[1],
    ...
    /**
     * Get versions from configuration.
     */
    protected function getConfiguredVersions(): array
    {
        $configured = config('api-response.openapi.versioning.versions', []);
        $versions = [];

        foreach ($configured as $key => $config) {
            $versions[$key] = [
                'name' => $key,
                'title' => $config['title'] ?? 'API ' . strtoupper($key),
                'description' => $config['description'] ?? '',
                'pattern' => $config['pattern'] ?? 'api/' . $key . '/*',
            ];
        }
    ...
    public function getDefaultVersion(): ?string
    {
        ...
        $defaultVersion = config('api-response.openapi.versioning.default_version');

        if ($defaultVersion && isset($versions[$defaultVersion])) {
     ...
     */
    protected function initializeSpec(?string $version = null): void
    {
        $baseTitle = config('api-response.openapi.title', config('app.name', 'API') . ' Documentation');
        $baseDescription = config('api-response.openapi.description', 'API documentation generated by Laravel API Response Builder');
        $baseVersion = config('api-response.openapi.version', '1.0.0');

        // Customize title/description for versioned docs
        if ($version) {
            $versions = $this->getAvailableVersions();
            if (isset($versions[$version])) {
                $versionInfo = $versions[$version];
                $baseTitle = $versionInfo['title'] ?? $baseTitle . ' - ' . strtoupper($version);
                if (! empty($versionInfo['description'])) {
                    $baseDescription = $versionInfo['description'];
                }
                $baseVersion = $version;
            }
        }
     ...
     */
    protected function processRoutes(?string $version = null): void
    {
        $routes = $this->router->getRoutes();
        $tags = [];
        $prefix = config('api-response.openapi.route_prefix', 'api');
        ...
        foreach ($routes as $route) {
            ...
            if ($version && ! $this->routeMatchesVersion($uri, $version, $prefix)) {
                continue;
            }
    ...
    /**
     * Check if a route URI matches the specified version.
     */
    protected function routeMatchesVersion(string $uri, string $version, string $prefix): bool
    {
        $versioningConfig = config('api-response.openapi.versioning', []);

        // Check custom version patterns first
        if (! ($versioningConfig['auto_detect'] ?? true)) {
            $versions = $versioningConfig['versions'] ?? [];
            if (isset($versions[$version]['pattern'])) {
                $pattern = $versions[$version]['pattern'];
        ...
        // Default: match version prefix pattern (e.g., api/v1/*)
        $versionPattern = $prefix . '/' . $version;
        return str_starts_with($uri, $versionPattern);
    }
